...
public class NotifAuthManager {
    ...
    public NotifAuth getNotifAuth(long user_id, int notif_code) {
        // Retourne la NotifAuth dont l'id est passé en paramètre
        NotifAuth a = new NotifAuth(0,0,0,0);

        Cursor c = db.rawQuery("SELECT * FROM "+TABLE_NAME+" WHERE "+KEY_NOTIFAUTH_USER_ID+"="+user_id+" AND "+KEY_NOTIFAUTH_NOTIF_CODE+"="+notif_code, null);
        if (c.moveToFirst()) {
